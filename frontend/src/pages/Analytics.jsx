...
const NATIONALITY_FLAGS = {
  Egyptian: 'eg', Egypt: 'eg', Pakistani: 'pk', Pakistan: 'pk',
  Indian: 'in', India: 'in', Bangladeshi: 'bd', Bangladesh: 'bd',
  Turkish: 'tr', Turkey: 'tr', Ethiopian: 'et', Ethiopia: 'et',
  Ugandan: 'ug', Uganda: 'ug', Moroccan: 'ma', Morocco: 'ma',
};
const flagUrl = (nat) => {
...
const CHART_TOOLTIP = { contentStyle: { background: '#0f1a2e', border: '1px solid #2b3a5a', color: '#e6eefb' } };
const CHART_AXIS    = { tick: { fill: '#8ba3b8', fontSize: 11 }, stroke: '#2b3a5a' };
const CHART_GRID    = { strokeDasharray: '3 3', stroke: '#1f2b45' };
...
function BrandedBarList({ data, mode, onClick }) {
  if (!data || data.length === 0) return <div className="xchart-empty">No data</div>;
  ...
  return (
    <div className="xbarlist">
      {data.map((d, i) => {
        ...
        const bg    = flag  ? `linear-gradient(90deg, rgba(12,22,40,0.10), rgba(12,22,40,0.25)), url(${flag}) center/cover no-repeat`
                    : brand ? brand.color
                    : 'linear-gradient(90deg, #2563eb, #38bdf8)';
        return (
          <div key={i} className="xbarlist-row" onClick={() => onClick && onClick(d.name)}>
            <div className="xbarlist-name">
              {brand && <img src={brand.logo} alt="" className="xbarlist-logo" onError={e => { e.target.style.display = 'none'; }} />}
              <span>{d.name}</span>
            </div>
            <div className="xbarlist-track">
              <div className="xbarlist-fill"
                   style={{ width: `${pct}%`, background: bg, borderColor: brand ? brand.accent : 'transparent' }}>
  ...
  return (
    <div className="xheatmap">
      <div className="xheatmap-row xheatmap-head">
        ...
        {Array.from({ length: 24 }).map((_, h) => <div key={h} className="xheatmap-hlab">{h}</div>)}
      </div>
      {dayNames.map((d, di) => (
...
export default function Analytics() {
  ...
  useEffect(() => { localStorage.setItem('exec_widgets', JSON.stringify(widgets)); }, [widgets]);
  useEffect(() => { const t = setInterval(() => setClock(new Date()), 1000); return () => clearInterval(t); }, []);
  useEffect(() => { getFilterOptions().then(setFilterOpts).catch(() => {}); }, []);
  ...
  async function reload() {
    ...
    try {
      ...
      setData(await getDashboardData(payload));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const onDrill      = (field, name) => setDrill({ field, value: name, label: name });
  const clearDrill   = () => setDrill(null);
  const clearAllFilters = () => { setShift(''); setStatus(''); setAirline(''); setNat(''); setDest(''); setPaxType(''); setDrill(null); };
  const toggleWidget = (id) => setWidgets(prev => ({ ...prev, [id]: !prev[id] }));
  const allWidgets   = (val) => setWidgets(Object.fromEntries(WIDGETS.map(w => [w.id, val])));
  ...
  function pickKpi(id, label) {
    if (kpiFilter?.id === id) { setKpiFilter(null); return; }
    const now = Date.now(), H12 = 12 * 3600000, H24 = 24 * 3600000;
    const preds = {
  ...
  const drillReports = useMemo(() => {
    ...
    return kpiFilter ? data.reports.filter(kpiFilter.predicate) : data.reports;
  }, [data, kpiFilter]);
    ...
    <div className="exec-dashboard">
      <header className="xheader">
        ...
        <div className="xheader-center">
          <div className="xchips">
            {['today','yesterday','week','month','year','all','custom'].map(r => (
              <button key={r} className={`xchip ${range === r ? 'active' : ''}`} onClick={() => setRange(r)}>
                {RANGE_LABELS[r]}
              </button>
            ))}
          </div>
        ...
        <div className="xheader-right">
          ...
          <button className="xiconbtn" onClick={() => setWidgetDrawer(!widgetDrawer)} title="Configure visible widgets">⚙</button>
          <button className="xiconbtn" onClick={() => navigate('/dashboard')} title="Back to Dashboard">←</button>
        </div>
      </header>
        ...
        <div className="xdrawer-overlay" onClick={() => setWidgetDrawer(false)}>
          <div className="xdrawer" onClick={e => e.stopPropagation()}>
            ...
            <div className="xdrawer-foot">
              <button className="xchip" onClick={() => allWidgets(true)}>Show all</button>
              <button className="xchip" onClick={() => allWidgets(false)}>Hide all</button>
            </div>
          </div>
        </div>
      )}
      ...
      {data && kpi && (
        <>
          <section className="xkpi-strip">
            {on('kpi_underProcess') && (
              <KpiCard label="UNDER PROCESS" value={kpi.underProcessCases}
                cases={kpi.underProcessCases} pax={kpi.underProcessPax}
            ...
            {on('kpi_nusuk') && (
              <KpiCard label="NUSUK INTERVENTION" value={kpi.needsNusukCases}
                cases={kpi.needsNusukCases} pax={kpi.needsNusukPax}
                status={kpi.needsNusukCases > 0 ? '⚠ Notify Ministry' : '✓ Up to date'}
                color="#1abc9c" accent="#26d7b3" flash={kpi.needsNusukCases > 0}
                active={kpiFilter?.id === 'kpi_nusuk'}
                onClick={() => pickKpi('kpi_nusuk', 'Nusuk Intervention')} />
            )}
            {on('kpi_departedRecent') && (
              <KpiCard label="DEPARTED · LAST 12H" value={kpi.departedRecentCases}
                cases={kpi.departedRecentCases} pax={kpi.departedRecentPax}
                status="Process completed" color="#4ade80" accent="#86efac"
                active={kpiFilter?.id === 'kpi_departedRecent'}
                onClick={() => pickKpi('kpi_departedRecent', 'Departed (last 12h)')} />
            )}
            {on('kpi_atAirportSoon') && (
              <KpiCard label="NEW FLIGHT < 12H" value={kpi.atAirportSoonCases}
                cases={kpi.atAirportSoonCases} pax={kpi.atAirportSoonPax}
                status="At airport · departing soon" color="#fb923c" accent="#fdba74"
                active={kpiFilter?.id === 'kpi_atAirportSoon'}
                onClick={() => pickKpi('kpi_atAirportSoon', 'New flight < 12h')} />
            )}
            {on('kpi_atAirportLater') && (
              <KpiCard label="NEW FLIGHT ≥ 12H" value={kpi.atAirportLaterCases}
                cases={kpi.atAirportLaterCases} pax={kpi.atAirportLaterPax}
                status="At airport · 12h+ wait" color="#60a5fa" accent="#93c5fd"
                active={kpiFilter?.id === 'kpi_atAirportLater'}
                onClick={() => pickKpi('kpi_atAirportLater', 'New flight ≥ 12h')} />
            )}
            {on('kpi_stuck24') && (
              <KpiCard label="AT AIRPORT > 24H" value={kpi.stuck24Cases}
                cases={kpi.stuck24Cases} pax={kpi.stuck24Pax}
                status={kpi.stuck24Cases > 0 ? '⚠ Extended stay' : '✓ None'}
                color="#ef476f" accent="#ff7a9e" flash={kpi.stuck24Cases > 0}
                active={kpiFilter?.id === 'kpi_stuck24'}
                onClick={() => pickKpi('kpi_stuck24', 'At airport > 24h')} />
            )}
            {on('kpi_confirmed') && (
              <KpiCard label="FLIGHT CONFIRMED" value={kpi.confirmedCases}
                cases={kpi.confirmedCases} pax={kpi.confirmedPax}
                status="Awaiting departure" color="#22c55e"
                active={kpiFilter?.id === 'kpi_confirmed'}
                onClick={() => pickKpi('kpi_confirmed', 'Flight Confirmed')} />
            )}
            {on('kpi_closed') && (
              <KpiCard label="CLOSED" value={kpi.closedCases}
                cases={kpi.closedCases} pax={kpi.closedPax}
                status="Departed" color="#64748b"
                active={kpiFilter?.id === 'kpi_closed'}
                onClick={() => pickKpi('kpi_closed', 'Closed')} />
            )}
            {on('kpi_total') && (
              <KpiCard label="TOTAL CASES" value={kpi.totalCases}
                cases={kpi.totalCases} pax={kpi.totalPax}
                status={RANGE_LABELS[range] || 'Range'} spark={kpi.sparkCases} color="#39d0d8"
                active={kpiFilter?.id === 'kpi_total'}
                onClick={() => pickKpi('kpi_total', 'All Cases')} />
            )}
            {on('kpi_rebook') && (
              <KpiCard label="AVG REBOOK TIME" value={fmtHrs(kpi.avgRebookHrs)}
                sub="created → confirmed"
                status={kpi.avgRebookHrs == null ? '—' : kpi.avgRebookHrs < 12 ? '▼ Within SLA' : '▲ Above SLA'}
                color="#f5c842" />
            )}
            {on('kpi_close') && (
              <KpiCard label="AVG CLOSE TIME" value={fmtHrs(kpi.avgCloseHrs)}
                sub="created → closed" status="— Stable" color="#38bdf8" />
            )}
            {on('kpi_days') && (
              <KpiCard label="AVG DAYS AT AIRPORT"
                value={kpi.avgDaysAtAirport != null ? kpi.avgDaysAtAirport.toFixed(1) : '—'}
                sub="days per case"
                status={kpi.avgDaysAtAirport != null && kpi.avgDaysAtAirport > 3 ? '▲ Review extended stays' : '— Normal'}
                spark={kpi.sparkDays} color="#b794f4" />
            )}
          </section>
          ...
          <section className="xgrid">
            {on('chart_nationality') && (
              <div className="xpanel xspan-6">
                <div className="xpanel-head">
                  ...
                  {data.byNationality.most && <span className="xtag xtag-hot">🔥 {data.byNationality.most.name} ({data.byNationality.most.value})</span>}
                </div>
                <BrandedBarList data={data.byNationality.data} mode="nationality" onClick={name => onDrill('nationality', name)} />
              </div>
            )}
            ...
            {on('chart_airline') && (
              <div className="xpanel xspan-6">
                <div className="xpanel-head">
                  <h3>BY AIRLINE</h3>
                  {data.byAirline.most && <span className="xtag xtag-hot">🔥 {data.byAirline.most.name} ({data.byAirline.most.value})</span>}
                </div>
                <BrandedBarList data={data.byAirline.data} mode="airline" onClick={name => onDrill('airline', name)} />
              </div>
            )}
            ...
            {on('chart_destination') && (
              <div className="xpanel xspan-4">
                <div className="xpanel-head">
                  ...
                  {data.byDestination.most && <span className="xtag">{data.byDestination.data.slice(0,3).map(d => d.name).join(' · ')}</span>}
                </div>
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={data.byDestination.data}>
                      ...
                      <Bar dataKey="value" fill="#38bdf8" radius={[4,4,0,0]}
                           onClick={d => onDrill('destination', d.name)} style={{ cursor: 'pointer' }}/>
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={data.byShift.data}>
                    ...
                    <Bar dataKey="value" radius={[4,4,0,0]}
                      ...
                      {data.byShift.data.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={data.byPaxType.data} dataKey="value" nameKey="name"
                        ...
                        {data.byPaxType.data.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} stroke="#0b1220" />)}
                      </Pie>
                      <Tooltip {...CHART_TOOLTIP}/>
                ...
                <div className="xpanel-head">
                  ...
                  {kpiFilter ? (
                    <span className="xtag xtag-hot" style={{ cursor: 'pointer' }} onClick={clearKpiFilter}>
                      Filtered: {kpiFilter.label} ({drillReports.length}) ✕
                    </span>
                  ) : (
                    <span className="xtag">{drillReports.length} reports · click a card or row</span>
                  )}
                </div>
                {drillReports.length === 0 ? <div className="xchart-empty">No reports match current filters</div> : (
