...
function getTerminal(flight) { return TERMINAL_MAP[getAirlineCode(flight)] || 'T1'; }
function needsBus(flight)    { const t = getTerminal(flight); return t === 'North' || t === 'Hajj'; }

function iataCode(dest) {
  if (!dest) return '???';
  const m = dest.match(/\(([A-Z]{3})\)/);
  return m ? m[1] : dest.slice(0, 3).toUpperCase();
}
...
function buildWhatsApp({ id, prev_flight, prev_destination, pax_count, pax_type, nationality, new_flight, new_datetime }) {
  return `No-Show Report #${id}\n` +
...
function computeDays(days_at_airport, prev_datetime) {
  let days = parseFloat(days_at_airport) || null;
  if (!days && prev_datetime) {
    const diff = (Date.now() - jeddahDtMs(prev_datetime)) / (1000 * 60 * 60 * 24);
    if (!isNaN(diff) && diff >= 0) days = parseFloat(Math.max(0, diff).toFixed(2));
  }
  return days;
}

async function stampStatus(pool, id, oldStatus, newStatus, existing) {
  if (oldStatus !== 'flight_confirmed' && newStatus === 'flight_confirmed' && !existing.confirmed_at)
    await pool.query('UPDATE reports SET confirmed_at = $1 WHERE id = $2', [jeddahNowStr(), id]);
  if (oldStatus !== 'closed' && newStatus === 'closed' && !existing.closed_at)
...
router.get('/handover', async (req, res) => {
  try {
    ...
    } else if (jeddahHour >= 6 && jeddahHour < 14)  { currentShift = 'A'; nextShift = 'B'; }
    else if (jeddahHour >= 14 && jeddahHour < 22)   { currentShift = 'B'; nextShift = 'C'; }
    else                                              { currentShift = 'C'; nextShift = 'A'; }
    ...
    function fmtReport(r, showNewFlight = false) {
      ...
      let line = `${count}PAX ${paxType} ${getAirlineCode(r.prev_flight)} ${dest} ${fmtDateShort(r.prev_datetime)} STD ${fmtTimeShort(r.prev_datetime)}`;
      if (showNewFlight && r.status === 'flight_confirmed' && r.new_flight) {
        ...
        line += `\n- ALT FLT ${r.new_flight} STD ${fmtTimeShort(r.new_datetime)} ${fmtDateShort(r.new_datetime)} ✅${termNote}${bus}`;
      }
      if (r.comment) line += `\n   → ${r.comment}`;
      return line + ` #${r.id}`;
    }
    ...
    const lines = [
      `📋 SHIFT HANDOVER ${currentShift} → ${nextShift}`,
      `${fmtDateShort(nowJeddah)} ${fmtTimeShort(nowJeddah)}`,
      '',
    ];
    ...
    const sections = [
      ['━━ UNDER PROCESS SV ━━',              upSV,    false],
      ['━━ UNDER PROCESS OTHER AIRLINES ━━',  upOther, false],
      ['━━ FOLLOW UP SV ━━',                  fuSV,    true],
      ['━━ FOLLOW UP OTHER AIRLINES ━━',      fuOther, true],
    ];
    sections.forEach(([heading, list, showNew]) => {
      if (!list.length) return;
      lines.push(heading, '');
      list.forEach(r => lines.push(fmtReport(r, showNew)));
      lines.push('');
    });
    ...
    lines.push(
      '━━ SUMMARY ━━',
      `Under Process: ${underProcess.length} cases (${totalUp} PAX)`,
      `Flight Confirmed: ${flightConfirmed.length} cases (${totalFu} PAX)`,
      `Total Active: ${underProcess.length + flightConfirmed.length} cases (${totalUp + totalFu} PAX)`,
    );
...
router.get('/shift-summary', async (req, res) => {
  try {
    const pool       = getDb();
    const targetDate = req.query.date || jeddahISO().slice(0, 10);
    ...
    for (const [shiftName, range] of Object.entries(shifts)) {
      ...
      const lines    = rows.map(r => `${String(r.pax_count||1).padStart(2,'0')}PAX Identified at ${(r.pax_id_datetime||'??:??').slice(11,16)}`);
      const totalPax = rows.reduce((sum, r) => sum + (r.pax_count || 1), 0);
      result[shiftName] = {
        lines, totalPax, totalReports: rows.length,
        text: lines.length > 0
...
router.post('/', upload.array('files', 10), async (req, res) => {
  try {
    ...
    const {
      ...
      days_at_airport, pax_count, submitted_by, status, comment,
    } = req.body;

    const filePaths    = (req.files || []).map(f => `/uploads/${f.filename}`);
    const reportStatus = status || 'under_process';
    const computedDays = computeDays(days_at_airport, prev_datetime);
    ...
    const { rows } = await pool.query(
      ...
      [
        ...
        computedDays, parseInt(pax_count) || 0,
        JSON.stringify(filePaths), '', submitted_by, reportStatus, comment || '', jeddahNowStr(),
      ]
    );

    const id = rows[0].id;
    const whatsapp_text = buildWhatsApp({ id, prev_flight, prev_destination, pax_count, pax_type, nationality, new_flight, new_datetime });
...
router.put('/:id', upload.array('files', 10), async (req, res) => {
  try {
    ...
    const {
      pax_id_datetime, prev_flight, prev_datetime, prev_destination, prev_airline,
      nationality, pax_type, new_flight, new_datetime, new_destination, new_airline,
      days_at_airport, pax_count, status, comment,
    } = req.body;

    const reportStatus = status || existing.status || 'under_process';
    const oldPaths     = JSON.parse(existing.file_paths || '[]');
    const filePaths    = req.files?.length ? [...oldPaths, ...req.files.map(f => `/uploads/${f.filename}`)] : oldPaths;
    const computedDays = computeDays(days_at_airport, prev_datetime);
    const whatsapp_text = buildWhatsApp({ id: existing.id, prev_flight, prev_destination, pax_count, pax_type, nationality, new_flight, new_datetime });
    ...
    await pool.query(
      ...
      [
        ...
        nationality, pax_type,
        new_flight || null, new_datetime || null, new_destination || null, new_airline || null,
        computedDays, parseInt(pax_count) || 0, JSON.stringify(filePaths), whatsapp_text,
        reportStatus, comment || '', req.params.id,
      ]
    );

    await stampStatus(pool, req.params.id, existing.status, reportStatus, existing);
    ...
    if (changes) await logAudit({ user: req.body.submitted_by || 'staff', action: 'edit', reportId: updated.id, changes });
    res.json(updated);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.patch('/:id', express.json(), async (req, res) => {
  try {
    ...
    if (new_flight !== undefined)      addField('new_flight', new_flight);
    if (new_datetime !== undefined)    addField('new_datetime', new_datetime);
    if (new_destination !== undefined) addField('new_destination', new_destination);
    if (new_airline !== undefined)     addField('new_airline', new_airline);
    if (comment !== undefined)         addField('comment', comment);
    if (pax_count !== undefined)       addField('pax_count', parseInt(pax_count) || 0);

    if (report.prev_datetime) {
      const diff = (Date.now() - jeddahDtMs(report.prev_datetime)) / (1000 * 60 * 60 * 24);
      if (!isNaN(diff)) addField('days_at_airport', parseFloat(Math.max(0, diff).toFixed(2)));
    }
    ...
    addField('whatsapp_text', buildWhatsApp({
      id: report.id, prev_flight: report.prev_flight, prev_destination: report.prev_destination,
      pax_count: finalPaxCount, pax_type: report.pax_type, nationality: report.nationality,
      new_flight: finalNewFlight, new_datetime: finalNewDatetime,
    }));
    ...
    await stampStatus(pool, req.params.id, report.status, newStatus, report);
...
router.post('/:id/files', upload.array('files', 10), async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      action: 'attach_files', reportId: rows[0].id,
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    try {
      JSON.parse(report.file_paths || '[]').forEach(fp => {
